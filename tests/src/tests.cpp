...
TEMPLATE_TEST_CASE("constructors", "[dynamic_bitset]", uint16_t, uint32_t, uint64_t)
{
    ...
    SECTION("initializer_list constructor")
    {
        SECTION("one value")
        {
            const TestType init_value = GENERATE(
              take(RANDOM_VECTORS_TO_TEST,
                   random<TestType>(std::numeric_limits<TestType>::min(), std::numeric_limits<TestType>::max())));
        ...
        SECTION("two values")
        {
            const std::tuple<TestType, TestType> init_values =
              GENERATE(multitake(RANDOM_VECTORS_TO_TEST,
                                 random<unsigned long long>(std::numeric_limits<unsigned long long>::min(),
                                                            std::numeric_limits<unsigned long long>::max()),
                                 random<unsigned long long>(std::numeric_limits<unsigned long long>::min(),
                                                            std::numeric_limits<unsigned long long>::max())));
            std::initializer_list<TestType> init_values_list = {std::get<0>(init_values), std::get<1>(init_values)};
            ...
            for(const TestType& value: init_values_list)
            {
                for(size_t bit_i = 0; bit_i < bits_number<TestType>; ++bit_i)
                {
                    ...
                    REQUIRE(bitset[value_i * bits_number<TestType> + bit_i] == bit_value(value, bit_i));
                }
                ++value_i;
            }
            REQUIRE(check_consistency(bitset));
    ...
    SECTION("string constructor")
    {
        ...
        {
            const std::tuple<size_t, size_t> parameters =
            ...
            const size_t n = (pos == (bits_to_take - 1)) ? std::get<1>(parameters) % (bits_to_take - pos) : 0;
            CAPTURE(pos, n);
...
{
    sul::dynamic_bitset<TestType> bitset = GENERATE(take(RANDOM_VECTORS_TO_TEST, randomDynamicBitset<TestType>()));
    CAPTURE(bitset);
...
TEMPLATE_TEST_CASE("pop_back", "[dynamic_bitset]", uint16_t, uint32_t, uint64_t)
{
    ...
    {
        std::tuple<sul::dynamic_bitset<TestType>, size_t> values = GENERATE(multitake(
          RANDOM_VECTORS_TO_TEST, randomDynamicBitset<TestType>(), random<TestType>(1, 2 * bits_number<TestType>)));
        sul::dynamic_bitset<TestType>& bitset = std::get<0>(values);
    ...
    SECTION("two values initializer_list")
    {
        const std::tuple<TestType, TestType> init_values =
          GENERATE(multitake(RANDOM_VARIATIONS_TO_TEST,
                             ...
                             random<unsigned long long>(std::numeric_limits<unsigned long long>::min(),
                                                        std::numeric_limits<unsigned long long>::max())));
        std::initializer_list<TestType> values = {std::get<0>(init_values), std::get<1>(init_values)};
        CAPTURE(values);
    ...
    SECTION("random access iterators")
    {
        const std::vector<TestType> values = GENERATE(
          take(RANDOM_VARIATIONS_TO_TEST,
               randomChunk<TestType>(
                 2, 5, random<TestType>(std::numeric_limits<TestType>::min(), std::numeric_limits<TestType>::max()))));
        CAPTURE(values);
        ...
        SECTION("binary operators")
        {
            ...
            SECTION("operator>>")
            {
                ...
                const unsigned long long shifted_value =
                  (value & (~static_cast<unsigned long long>(0) >> (bits_number<unsigned long long> - bits_to_take)))
                  >> shift;
...
TEMPLATE_TEST_CASE("all any none", "[dynamic_bitset]", uint16_t, uint32_t, uint64_t)
{
    ...
    SECTION("non-empty bitset")
    {
        const size_t bitset_size =
          GENERATE(take(RANDOM_VECTORS_TO_TEST, random<size_t>(3 * bits_number<TestType>, 8 * bits_number<TestType>)));
        CAPTURE(bitset_size);
...
TEMPLATE_TEST_CASE("size num_blocks empty capacity", "[dynamic_bitset]", uint16_t, uint32_t, uint64_t)
{
    const std::tuple<unsigned long long, size_t> values =
      GENERATE(multitake(RANDOM_VECTORS_TO_TEST,
                         random<unsigned long long>(std::numeric_limits<unsigned long long>::min(),
    ...
    const size_t num_blocks =
      bits_to_take / bits_number<TestType> + static_cast<size_t>(bits_to_take % bits_number<TestType> > 0);
...
{
    const size_t size = GENERATE(take(RANDOM_VARIATIONS_TO_TEST, random<size_t>(1, 8 * bits_number<TestType>)));
    CAPTURE(size);
...
TEMPLATE_TEST_CASE("is_subset_of is_proper_subset_of", "[dynamic_bitset]", uint16_t, uint32_t, uint64_t)
{
    sul::dynamic_bitset<TestType> bitset = GENERATE(take(RANDOM_VECTORS_TO_TEST, randomDynamicBitset<TestType>()));
    CAPTURE(bitset);
...
TEMPLATE_TEST_CASE("swap", "[dynamic_bitset]", uint16_t, uint32_t, uint64_t)
{
    const std::tuple<sul::dynamic_bitset<TestType>, sul::dynamic_bitset<TestType>> bitsets =
      GENERATE(multitake(RANDOM_VECTORS_TO_TEST, randomDynamicBitset<TestType>(), randomDynamicBitset<TestType>()));
...
TEMPLATE_TEST_CASE("to_ullong", "[dynamic_bitset]", uint16_t, uint32_t, uint64_t)
{
    ...
    SECTION("non-empty bitset")
    {
        ...
        SECTION("overflow exception")
        {
            ...
            size_t last = sul::dynamic_bitset<TestType>::npos;
            bitset.iterate_bits_on([&last](size_t bit_pos) noexcept { last = bit_pos; });
            if(last == sul::dynamic_bitset<TestType>::npos || last <= bits_number<unsigned long long>)
            {
                do
                {
                    bitset.push_back(true);
...
TEMPLATE_TEST_CASE("iterate_bits_on", "[dynamic_bitset]", uint16_t, uint32_t, uint64_t)
{
    ...
    SECTION("return void")
    {
        ...
        SECTION("parameters")
        {
            ...
            bitset.iterate_bits_on([](size_t bit_pos, sul::dynamic_bitset<TestType>& check_bitset_)
...
TEMPLATE_TEST_CASE("operator== operator!=", "[dynamic_bitset]", uint16_t, uint32_t, uint64_t)
{
    ...
    SECTION("same size and different bits")
    {
        ...
        size_t pos = GENERATE(take(RANDOM_VARIATIONS_TO_TEST,
                                   random<unsigned long long>(std::numeric_limits<unsigned long long>::min(),
                                                              std::numeric_limits<unsigned long long>::max())))
                     % bitset.size();
        CAPTURE(pos);
...
TEMPLATE_TEST_CASE("operator< operator<= operator> operator>=", "[dynamic_bitset]", uint16_t, uint32_t, uint64_t)
{
        ...
        SECTION("different size and different bits")
        {
            SECTION("difference in common blocks")
            {
                ...
                const size_t bits_to_add =
                  GENERATE(take(RANDOM_VARIATIONS_TO_TEST, random<size_t>(0, 2 * bits_number<TestType>)));
            ...
            SECTION("difference in extra blocks")
            {
                ...
                sul::dynamic_bitset<TestType> bits_to_add = GENERATE(
                  take(RANDOM_VARIATIONS_TO_TEST, randomDynamicBitset<TestType>(1, 2 * bits_number<TestType>)));
                CAPTURE(bits_to_add);
                if(bits_to_add.none())
                {
                    bits_to_add.set(0);
