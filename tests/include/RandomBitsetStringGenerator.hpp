...
inline RandomBitsetStringGenerator::RandomBitsetStringGenerator(size_type min_size,
                                                                size_type max_size,
                                                                uint32_t seed)
  : m_rand(seed), m_size_dist(min_size, max_size), m_bit_dist('0', '1'), m_current_bitset()
{
	next();
}
...
inline Catch::Generators::GeneratorWrapper<std::string> randomBitsetString(
  std::string::size_type min_size,
  std::string::size_type max_size,
  uint32_t seed)
{
